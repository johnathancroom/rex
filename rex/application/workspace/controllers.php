<?php
// rex/application/workspace/controllers.php
...
class handle {
  ...
  public function load($identifier, $data = NULL) {
    ...
    if ($this->exists($identifier) === true) {
      $this->rex->core->modules->log->_('Failed to load controller \'' . $identifier . '\' because the identifier is already set');
      return false;
    }
    $file = $this->data['path'] . '/' . $identifier . '.php';
    if (file_exists($file) === false) {
      $this->rex->core->modules->log->_('Failed to load controller \'' . $identifier . '\' because the file \'' . $file . '\' does not exist');
      return false;
    }
    include $file;
    $class = __NAMESPACE__ . '\\' . $identifier;
    if (class_exists($class) === false) {
      $this->rex->core->modules->log->_('Failed to load controller \'' . $identifier . '\' because the class \'' . $class . '\' is not defined in the file \'' . $file . '\'');
      return false;
    }
    $this->$identifier = new $class($this, $data);
      $this->rex->core->modules->log->_('Loaded controller \'' . $identifier . '\'');
    return true;
  }
}
class controller {
